...
freq = [0.3550:0.0000001:0.3650];
numFreqs = length(freq);
analyticalMaxPop = zeros(1,numFreqs);

E_0 = 1E-6;

for i = 1:numFreqs
    omega_r = 0.5*sqrt((omega_0-freq(i))^2+(E_0^2*p_z^2)/omega_0^2);
    analyticalMaxPop(i) = E_0^2*p_z^2/(4*omega_0^2*omega_r^2);
end

f1 = figure('Visible','off','Position',[0 0 1618 1000]);
hold on;
plot(freq,analyticalMaxPop,'LineWidth',8,'Color',[0.9290 0.6940 0.1250]);
%title('Maximum Excited State Population');
xlabel('Driving Frequency \omega (a.u.)');
ylabel('Maximum E.S. Population');
set(gca,'FontSize',25);
xlim([0.355,0.365]);
yscale("log");

for omega = [0.355,0.356,0.357,0.358,0.359,0.361,0.362,0.363,0.364]
    omegaString = num2str(omega);
    nonRWApath = strcat("NoRWA/E0.000001_w",omegaString,"/dataFile.txt");
    RWApath = strcat("RWA/E0.000001_w",omegaString,"/dataFile.txt");
    nonRWAdata = importdata(nonRWApath," ");
    RWAdata = importdata(RWApath," ");
    nonRWAlen = length(nonRWAdata.textdata(:,7));
    RWAlen = length(RWAdata.textdata(:,7));
    if nonRWAlen > RWAlen
        nonRWApop = zeros(RWAlen,1);
        RWApop = zeros(RWAlen,1);
        for i = 1:RWAlen
            nonRWApop(i) = str2double(cell2mat(nonRWAdata.textdata(i,7)));
            RWApop(i) = str2double(cell2mat(RWAdata.textdata(i,7)));
        end
        nonRWAmax = max(nonRWApop);
        RWAmax = max(RWApop);
        scatter(omega,nonRWAmax,'filled','MarkerFaceColor',[0 0.4470 0.7410],'SizeData',150);
        scatter(omega,RWAmax,'filled','MarkerFaceColor',[0.8500 0.3250 0.0980],'Marker','^','SizeData',150);
    else
        nonRWApop = zeros(nonRWAlen,1);
        RWApop = zeros(nonRWAlen,1);
        for i = 1:nonRWAlen
            nonRWApop(i) = str2double(cell2mat(nonRWAdata.textdata(i,7)));
            RWApop(i) = str2double(cell2mat(RWAdata.textdata(i,7)));
        end
...
legend('Analytical Solution',"Numerical non-RWA Solution", "Numerical RWA Solution",'Location','northwest');%%%%% Manually edit this line! %%%%%
figName = "plot_maxPop_vs_freq_E0.000001.png";
saveas(f1, figName);
...
for i = 1:maxDirs
    if dirStructs(i,1).name(1) ~= '.' & dirStructs(i,1).name ~= "" & dirStructs(i,1).isdir == 1
        ...
        if ~ismember(0, matchingIndices)%%%%% Only make plot if the files are present everywhere %%%%%
            ...
            omega_r = 0.5*sqrt((omega_0-omega)^2+E_0^2*p_z^2);
